...
from fastapi.responses import Response, JSONResponse # <<< CAMBIO: Ya no necesitamos HTMLResponse, FileResponse, RedirectResponse
from fastapi.middleware.cors import CORSMiddleware # <<< AÑADIDO: Middleware para CORS
...
app = FastAPI(
    title="ConversationRelay Translate API",
    description="Una API para realizar traducciones de voz en tiempo real entre dos llamadas telefónicas.",
    version="1.0.0"
)

# <<< CAMBIO CRÍTICO: Añadir Middleware de CORS >>>
# Esto es OBLIGATORIO para que un frontend en otro dominio pueda llamar a tu API.
origins = [
    # En desarrollo, puedes usar "*" para permitir todo
    "*", 
    # En producción, deberías ser más específico:
    # "https://tu-dominio-del-frontend.com",
    # "http://localhost:3000", # Si tu frontend corre localmente en el puerto 3000 (React, etc.)
]
...
# <<< CAMBIO: Se elimina el endpoint @app.get("/") que servía el HTML >>>
# La API ya no es responsable de servir el frontend.
...
async def initiate_call(request: Request):
    ...
    # <<< CAMBIO: La respuesta de error ahora es JSON, no HTML >>>
    ...
    if not twilio_number:
        return JSONResponse(
            status_code=500,
            content={"status": "error", "message": "Twilio phone number not configured"}
        )
    ...
    try:
        ...
        session.play_waiting_music = play_waiting_music
        ...
        # La respuesta de éxito ya era JSON, así que se mantiene igual.
        return JSONResponse(
            content={
                "status": "success",
